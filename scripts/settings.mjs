...
export function registerSettings() {
  ...
  game.settings.register("ai-dungeon-master-pf2e", "geminiModel", {
    ...
    choices: {
      ...
      "gemini-2.5-pro":
        "Gemini 2.5 Pro (2 req/min) 🎯 Avançado - Requer Conta Paga",
    },
  ...
  game.settings.register("ai-dungeon-master-pf2e", "maxTokens", {
    ...
    default: 8192,
    onChange: (value) => {
      const selectedModel = game.settings.get(
      ...
      if (selectedModel === "gemini-2.5-pro" && value < 4096) {
        ui.notifications.warn(
          "Gemini 2.5 Pro usa tokens de raciocínio. Recomendado: 8192+ tokens",
        );
      } else if (value < 2048) {
        ui.notifications.warn(
          "Valores abaixo de 2048 podem resultar em narrativas incompletas.",
        );
      }
    },
  });
